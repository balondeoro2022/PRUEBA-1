...
if isinstance(df, Exception):
    ...
    with tab1:
        st.header("3. Análisis de Medias, Máximos, Mínimos y Desviaciones")
        # 9 Desviaciones y medias diferentes (3 procesos x 3 sensores)
        stats = df.groupby(['proceso', 'sensor'])['valor_detectado'].agg(['mean', 'max', 'min', 'std', 'count']).reset_index()
        ...
        sns.boxplot(data=df, x='proceso', y='valor_detectado', hue='sensor', palette='Set2')
        plt.title("Dispersión de Lecturas por Proceso y Sensor")
        st.pyplot(fig_box)

    # 4. TABLAS DE FRECUENCIA
    with tab2:
        st.header("4. Tablas de Frecuencia y Distribución")
        c1, c2, c3 = st.columns(3)
        
        with c1:
            st.subheader("4.1 Fallas por Proceso")
            f_proc = df['proceso'].value_counts().reset_index()
        ...
        with c2:
            ...
            f_sens = df['sensor'].value_counts().reset_index()
            f_sens.columns = ['Sensor', 'Cant. Fallas']
            st.table(f_sens)
            
            fig_s = plt.figure(figsize=(5, 4))
            sns.countplot(data=df, x='sensor', palette='magma')
            plt.xticks(rotation=45)
            st.pyplot(fig_s)

        with c3:
            st.subheader("4.3 Falla más Común")
            f_tipo = df['tipo_falla'].value_counts().reset_index()
    ...
    # 5. GRÁFICOS DETALLADOS POR PROCESO (OBLIGATORIO)
    with tab3:
        st.header("5. Distribución Estadística por Proceso (Histogramas)")
        ...
        for proc in procesos:
            ...
            for i, (sens, unit, col, limit, lab) in enumerate(sensores_config):
                datos_f = df[(df['proceso'] == proc) & (df['sensor'] == sens)]['valor_detectado']
